...
const customFetchWithToken = async (
  ...
  const methodType = typeof args == "string" ? args : args.method;

  const isMutation =
    methodType && ["POST", "PUT", "DELETE", "PATCH"].includes(methodType.toUpperCase());

  if (!isMutation) {
    return baseQuery(args, api, extraOptions);
  }

  const csrfToken = api.getState().user.csrfToken;
